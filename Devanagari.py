# ============================================================
# Devanagari Character Set — PCA, KNN, K-Means (v2)
# ============================================================
import os
import glob
import numpy as np
...
path = kagglehub.dataset_download("rishianand/devanagari-character-set")
...
def infer_label(filepath, root):
    # Devanagari Character Set layout: root/.../character_xx_<name>/<image>.png
    # Immediate parent folder name works as the class label.
...
for ax, i in zip(axes.ravel(), sample_idx):
    ...
    ax.set_title(str(class_names[y[i]]), fontsize=8)
    ax.axis("off")
plt.suptitle("Sample Devanagari Characters (normalized polarity)")
plt.tight_layout()
plt.show()
